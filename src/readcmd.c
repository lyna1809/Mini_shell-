 ...
 */
 ...
 struct cmdline *readcmd(void)
 {
	 ...
	 s->out = 0;
	 s->seq = 0;
	 s->is_in_bg=0;        /*modification effectuée */

	int nb_words=0;
	// calcule nombre des mots 
	while (words[nb_words]!=0) {
		nb_words++;
	}

	if(nb_words!=0){
		if( words[nb_words-1] != NULL && strcmp(words[nb_words-1], "&") ==   0) {
			s->is_in_bg =   1;   
			words[nb_words-1] = NULL; // Supprime le '&' de la fin
		}
	}
